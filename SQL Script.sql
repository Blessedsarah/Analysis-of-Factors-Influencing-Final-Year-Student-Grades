...
-- Mean of the CGPA
SELECT CAST (AVG(semester_CGPA)AS DECIMAL (5,2)) 
AS Mean_CGPA
FROM Semester_CGPA
...
SELECT 
    Level_Semester,
   CAST(SUM(Grade_Point * Credit_Unit) AS DECIMAL)
	/SUM(Credit_Unit) AS semester_CGPA
	FROM Records
	GROUP BY
    Level_Semester 
...
WITH FailureRates AS(
	SELECT Course_Code,
			COUNT(CASE WHEN Score < 40 THEN 1 END)
			AS Failure_rate
	FROM Records
	GROUP BY Course_Code)
SELECT TOP 6 Course_Code, Failure_rate
FROM FailureRates
